...
PROGRAM_NAME = "Video Match Viewer"
VERSION = "0.1"
HOME_LINK = "https://github.com/op200/VideoMatch/tree/py_scheme"
...
if os_type == 'Windows':
    config_dir = os.path.join(os.getenv('APPDATA'), PROGRAM_NAME)
...
def save_config():
    config["DEFAULT"]["custom_options"] = cmp_custom_options_Entry.get()
...
cmp_custom_options_Frame = ttk.Frame(cmp_set_Frame)
cmp_custom_options_Frame.grid(row=0,column=0)

cmp_custom_options_Label = ttk.Label(cmp_custom_options_Frame, text="自定义选项:")
cmp_custom_options_Label.grid(row=0,column=0,padx=5)

cmp_custom_options_Entry = ttk.Entry(cmp_custom_options_Frame)
cmp_custom_options_Entry.insert(0,config.get("DEFAULT","custom_options"))
cmp_custom_options_Entry.grid(row=0,column=1)
...
def show_plt():
    ...
    def on_add(sel):
        # 文本样式
        x, y = sel.target
        sel.annotation.set_text(f"{'frame num:':<12}{str(x):>9}\n{'offset'+' '+sel.artist.get_label()+':':<12}{str(round(y,6)):>9}")
...
class algorithm:
    ...
    @staticmethod
    def get_r_data():
        algorithm.r_data = [0] * int(cv2.VideoCapture(input_video_path_2).get(cv2.CAP_PROP_FRAME_COUNT))
        index = 0
        for i in range(0, len(algorithm.data)):
            while algorithm.data[i] != -1 and index < algorithm.data[i]:
                algorithm.r_data[index] = -1
                index+=1
            if algorithm.data[i] != -1:
                algorithm.r_data[algorithm.data[i]] = i

    @staticmethod
    def listen_data():
        try:
            if input_video_path_now == input_video_path_1:
                show_data_Label.config(text=f"{frame_now} -> {algorithm.data[frame_now]}")
            else:
                show_data_Label.config(text=f"{frame_now} -> {algorithm.r_data[frame_now]}")
        except:
            return
    ...
    def get_plt():
        ...
        y = [algorithm.data[i]-i if algorithm.data[i]!=-1 else -1 for i in range(0,len(algorithm.data))]

        plt.plot(x, y)

        y_max = max(y)+1
        plt.ylim(-1, y_max)
        # plt.yticks([i for i in range(min(y), y_max)])
        plt.yticks([i for i in range(min(y), y_max)])
...
def flush_ffmpeg_speed_progress(frame):
    ...
    if frame >= frame_count:
        frame_now = frame_count-1
        log.warning(f"VideoMatch 解码的视频帧数({frame})与 cv2 解码的帧数({frame_count})不一致")
    else:
        frame_now = frame
    jump_to_frame()

def Thread_encoding():
    global process_VideoMatch
    algorithm.is_encoding = True
    log.info("开始")

    algorithm.data=[]

    process_VideoMatch = subprocess.Popen([
        'videomatch',
        '-i1', str(input_video_path_1),
        '-i2', str(input_video_path_2),
        ' '+cmp_custom_options_Entry.get()
    ],stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', errors='replace')

    while True:
        line = process_VideoMatch.stdout.readline()
        if not line and process_VideoMatch.poll() is not None:
            break
        if line:
            error = re.search(r"ERROR\][^ ]* (.*)$", line)
            if error:
                log.info(error.group(1))
                continue

            warning = re.search(r"WARNING\][^ ]* (.*)$", line)
            if warning:
                log.info(warning.group(1))
                continue

            match = re.match(r"(\d+)->(-?\d+)", line)
            if match:
                data = (int(match.group(1)), int(match.group(2)))
                log.info(f"{data[0]} -> {data[1]}")
                algorithm.data.append(data[1])
    process_VideoMatch.wait()


    algorithm.get_r_data()
